...
D        j = I
DI       k = I
DV       IF CHECKPASS(username, words(i) + CHR(j + XLVII) + CHR(k + XLVII)) THEN GOTO DXL
DVI      k = k + I
DVII     IF k < XI THEN GOTO DV
DX       j = j + I
DXV      IF j < XI THEN GOTO DI
DXX      i = i + I
DXXV     IF i <= pwdcount THEN GOTO D
DXXX     PRINT "no extended matches for user " + username + CHR(X)
DXXXV    END
DXL      PRINT "found match!! for user " + username + CHR(X)
DXLV     PRINT "password: " + words(i) + CHR(j + XLVII) + CHR(k + XLVII) + CHR(X)
DL       END
